...
重新思考这段代码。每一个 block 里有 TILE x TILE 个线程。每一列的d_K、每一行的d_Q都分别被取用了 TILE 次！

而直接从显存 HBM 中取用数据IO效率是很慢的。这就可以用到 SM 里的 shared_memory 共享内存，只从显存里读取一次数据写入共享内存，后续都从共享内存中读取。

所以我们需要两块 TILE x TILE 的 float 矩阵的共享内存空间。